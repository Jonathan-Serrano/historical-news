...
class UserInterestResource(Resource):
    ...
    def delete(self, user_id):
        ...
        return make_response(
            jsonify({"message": "Interest removed successfully!"}), 200
        )
...
class UserArticleResource(Resource):
    ...
    def delete(self, user_id):
        ...
        return make_response(
            jsonify({"message": "Article removed from saved list!"}), 200
        )
...
class ArticleTopicResource(Resource):
    def get(self):
        ...
        if not topic or not before_date:
            return make_response(
                jsonify({"error": "Missing 'topic' or 'before_date' parameter"}), 400
            )
        ...
        try:
            ...
            return make_response(
                jsonify({"error": "'before_date' must be in 'YYYY-MM-DD' format"}), 400
            )
